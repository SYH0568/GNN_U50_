...
HOST_SRCS += $(xcl2_SRCS)
CXXFLAGS += $(opencl_CXXFLAGS) -Wall -O0 -g -std=c++11 -DHLS4ML_DATA_DIR=./
...
$(TEMP_DIR)/alveo_hls4ml.xo: src/alveo_hls4ml.cpp
	mkdir -p $(TEMP_DIR)
	$(VPP) $(CLFLAGS) --temp_dir $(TEMP_DIR) -c -k alveo_hls4ml -I'$(<D)' -o'$@' '$<' ./src/myproject.cpp -I./src/ -I./src/weights  --config config.ini --kernel_frequency 200
